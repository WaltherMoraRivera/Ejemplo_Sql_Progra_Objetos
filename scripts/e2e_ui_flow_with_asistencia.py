#!/usr/bin/env python3
"""E2E test validating ASISTENCIA integration in UI flow without database access."""
...
def run_e2e_test():
    """Test ASISTENCIA entity in menu, table selection, and back navigation."""
    ...
    with patch('app.infrastructure.database.oracle_connection.OracleConnection.from_settings', return_value=mock_connection):
        with patch('app.infrastructure.repositories.asistencia_repository.AsistenciaRepository.get_all', return_value=[]):
            ...
            step1_menu_visible = window._main_layout.itemAt(0) is not None
            
            # Step 2: Select ASISTENCIA from menu
            ...
            # Step 4: Select another entity (FUNCION) to test widget reusability
            window._set_entity('funcion')
            ...
            # Step 6: Select ASISTENCIA again (widget must not be deleted)
            window._set_entity('asistencia')
            ...
            assert step2_entity == 'asistencia', "Should select asistencia entity"
            assert "Asistencias" in step2_title, "Title should indicate Asistencias"
            assert step2_actions_visible, "Actions should be visible in table view"
            assert step2_table_visible, "Table should be visible after selecting ASISTENCIA"
            ...
            assert step4_entity == 'funcion', "Should switch to funcion entity"
            assert step5_entity is None, "Entity should be None after back to menu again"
            assert step6_entity == 'asistencia', "Should select asistencia again"
            assert "Asistencias" in step6_title, "Final title should indicate Asistencias"
            assert step6_table_still_visible, "Table should still be usable after multiple transitions"
            
            print("✅ E2E UI Flow Test with ASISTENCIA PASSED: All transitions completed without errors.")
            window.close()
            return True
